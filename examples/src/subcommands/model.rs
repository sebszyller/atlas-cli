...
pub fn execute_model_action(
    ...
    match action {
        "create" => create_model(framework, step),
        "verify" => verify_model(framework, step),
        "list" => list_models(framework, step),
        _ => Err(AtlasError::UnknownAction(format!("model:{}", action))),
    }
}

fn create_model(framework: &mut AtlasTestFramework, step: &Step) -> Result<Option<String>> {
    let params = &step.parameters;
    let env = &framework.config.environment;

    let mut cmd = AtlasCommand::new("atlas-cli");
    cmd.add_subcommand("model", "create");
    ...
    tracing::debug!("Created model manifest: {}", manifest_id);
    Ok(Some(manifest_id))
}

fn verify_model(framework: &mut AtlasTestFramework, step: &Step) -> Result<Option<String>> {
    let params = &step.parameters;

    let mut cmd = AtlasCommand::new("atlas-cli");
    cmd.add_subcommand("model", "verify");
...
fn list_models(framework: &mut AtlasTestFramework, _step: &Step) -> Result<Option<String>> {
    let mut cmd = AtlasCommand::new("atlas-cli");
    cmd.add_subcommand("model", "list");
    ...
    if !stdout.trim().is_empty() {
        tracing::info!("Models found:\n{}", stdout);
    } else {
        tracing::info!("No models found in storage");
    }
